...
def has22(nums):
  cont = 0
  for numero in range(len(nums)-1):
    if nums[numero] == nums [numero + 1]:
      cont = cont + 1
  if cont > 0:
    return True
  if cont == 0:
...
def soma_na_lista(n, lista):
  for x in range(len(lista)-1):
    if lista[x] + lista[x+1] == n and lista[x] != lista[x+1]:
      return True
  for x in range(len(lista)-2):
    if lista[x] + lista[x+2] == n and lista[x] != lista[x+2]:
      return True
  if lista[0] + lista[3] == n and lista[0] != lista[3]:
      return True
  else:
    return False
...
def fila_tijolos(n_peq, n_gra, meta):
  return n_peq + n_gra*5 >= meta and meta%5 <= n_peq
